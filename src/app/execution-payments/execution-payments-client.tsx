...
import { createExecutionPayment } from "./actions";
...
export function ExecutionPaymentsClient({
  payments,
  bookings,
}: {
  payments: Payment[];
  ...
  const filtered = payments.filter((p) => {
    const matchesSearch =
      !search ||
      p.execution_company.name.includes(search) ||
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      formData.set(
        ...
        String(selectedBooking!.execution_company.id)
      );
      formData.set("amount", amount);
      formData.set("notes", notes);

      await createExecutionPayment(formData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">مدفوعات شركات التنفيذ</h1>
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((payment) => (
                    <TableRow key={payment.id}>
                      ...
                      <TableCell className="font-medium">
                        {payment.execution_company.name}
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>إضافة دفعة شركة تنفيذ جديدة</DialogTitle>
            <DialogDescription>
              اختر الحجز وأدخل مبلغ الدفع
            </DialogDescription>
          </DialogHeader>
                ...
                <SelectContent>
                  {bookings.map((b) => (
                    <SelectItem key={String(b.id)} value={String(b.id)}>
                      #{String(b.id)} - {b.execution_company.name}
                      {b.booking_reference
                        ? ` (${b.booking_reference})`
                        : ""}
                    </SelectItem>
            ...
            {selectedBooking && (
              <div className="rounded-lg border bg-muted/50 p-3 text-sm space-y-1">
                <div className="font-medium">
                  {selectedBooking.execution_company.name}
                </div>
              </div>
            )}
